...
class UserInput:
    ...
    # Create a location point and test if the user is within the Isle of Wight, else quit the application.
    def location(self):
        ...
        if 430000 <= self.easting <= 465000 and 80000 <= self.northing <= 95000:
            if p_i_p[0]:
                ...
                raise Exception('Error! Swim to shore or call 999 and ask for the coastguard')
        ...
        # Extend the region to the elevation raster bounds and test if the location is within the Isle of Wight.
            ...
            raise Exception('Error! You are not on or near the Isle of Wight')
...
class HighestPoint:
    def __init__(self, transform, buffer):
    ...
    def find_highest_point(self):
        ...
        out_tif_h = rasterio.open('elevation/out.tif')
        ...
        if bounds[0] <= highest_easting <= bounds[2] and bounds[1] <= highest_northing <= bounds[3]:
            print(f'The nearest highest point within the radius of 5km from your current location is {destination}')
            return destination
...
class NearestItn:
    def __init__(self, location, destination, buffer):
    ...
    def nearest_itn(self):
        ...
        isle_of_wight_itn_json = os.path.join('itn', 'solent_itn.json')
...
class ShortestPath:
    def __init__(self, start_point, end_point, buffer, transform):
